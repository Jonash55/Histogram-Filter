...
def move_right(mapa_size, move_precision_zostaje, move_precision_cel, move_precision_kolejna,
               move_precision_lewo_cel, move_precision_prawo_cel):
...
def move_up(mapa_size, move_precision_zostaje, move_precision_cel, move_precision_kolejna,
    ...
    for i in range(0, mapa_size):
        for j in range(0, 6):
            if i == 1:
                if j == 1:
                    probability[i][j] = probability[i][j] + probability_apriori[i - 1][j] * move_precision_lewo_cel
...
def move_down(mapa_size, move_precision_zostaje, move_precision_cel, move_precision_kolejna,
    ...
    for i in range(0, mapa_size):
        for j in range(0, 6):
            if i == 1:
                if j == 1:
                    probability[i][j] = probability[i][j] + probability_apriori[i + 1][j] * move_precision_prawo_cel
...
move_precision_cel = 0.6  # trafiamy do kratki docelowej
